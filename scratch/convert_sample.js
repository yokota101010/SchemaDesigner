...
// markdownGenerator.ts のロジックをJSに移植した簡易版
...
const targets = [
  {
    json: 'data/家計管理システム/家計管理システム_schema.json',
    md: 'data/家計管理システム/spec.md'
  },
  {
    json: 'models/sample1/Tutorial_Project_(Composite_Key_Demo)_schema.json',
    md: 'models/sample1/spec.md'
  }
];

targets.forEach(target => {
  try {
    const jsonPath = path.resolve(target.json);
    const mdPath = path.resolve(target.md);

    if (!fs.existsSync(jsonPath)) {
      console.log(`Target not found, skipping: ${jsonPath}`);
      return;
    }

    console.log(`Reading JSON from: ${jsonPath}`);
    const jsonRaw = fs.readFileSync(jsonPath, 'utf8');
    const projectData = JSON.parse(jsonRaw);

    console.log('Generating Markdown spec...');
    const mdContent = generateMarkdown(projectData);

    console.log(`Writing Markdown to: ${mdPath}`);
    fs.writeFileSync(mdPath, mdContent, 'utf8');

    console.log('Deleting original JSON file...');
    fs.unlinkSync(jsonPath);

    console.log(`Conversion completed: ${target.json} -> ${target.md}`);
  } catch (error) {
    console.error(`Error during conversion of ${target.json}:`, error);
  }
});
