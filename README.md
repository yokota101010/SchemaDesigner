...
graph TD
    ...
    B --> C[Phase 3: 未計算導出項目の初期化 (null設定)]
    C --> D[Phase 4: 依存関係に基づく段階的導出計算 (Pass 1..N)]
    D -->|未充填項目が存在する間ステップ評価| D
```
...
3. **未計算導出項目の初期化**: 生成された全レコードの導出項目（`dependent`）を一度 `null` に初期化します。
4. **依存関係に基づく段階的導出計算 (Topological Multi-Pass Derivation)**: 導出項目の依存関係（レベル別）に基づき、依存元の値がすべて確定（非 `null`）した計算可能項目から順番にAIへ計算リクエストを投入し、すべての導出項目が埋まるまで段階的に計算・充填を行います。これによりAIの認知負荷を激減させ、計算精度を最大化します。
5. **レートリミット対策とリトライ**: 429（Rate Limit）検知時は自動的に60秒待機に切り替え、最大3回（`maxRetries = 3`）のリトライを行います。クォータ超過（Quota Exceeded）検出時は安全に処理を打ち切ります。
