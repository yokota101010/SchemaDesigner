...
import { Table, UniqueKey, OrderBy, OrderByKey } from '../../../../../domain/models';

interface OrderBySettingsProps {
    editingTable: Table;
    tableUqs: UniqueKey[];
...
export const OrderBySettings: React.FC<OrderBySettingsProps> = ({ editingTable, tableUqs, updateTableOrderBy }) => {
    // 1. 主キー (PK)
    ...
    const currentOrderBy: OrderBy = editingTable.orderBy || { type: '', uqId: '' };

    // 選択されている制約に対応する構成カラムを特定
    let selectedConstraintColumns: typeof pkColumns = [];
    ...
    let sortedKeys: OrderByKey[] = currentOrderBy.keys || [];
