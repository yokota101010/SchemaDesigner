...
// --- UI側 (AiGeneratePromptModal.tsx) で参照する定数 ---
...
// --- 生成エンジン (aiDataGenerator.ts) で参照するプロンプト定数 ---

/**
 * AIに対するシステム側の基本的な役割定義 (System Role Prompt)
 */
export const AI_PROMPT_SYSTEM_ROLE = (tableName: string, tableId: string): string => 
 ...
 */
export const AI_PROMPT_USER_INSTRUCTIONS_HEADER = (aiInstructions: string, tableName: string, tableId: string): string => 
 ...
 */
export const AI_PROMPT_GENERATION_RULES = (rowCount: number, pkColumnId: string): string => `
 ...
 * 第2段階用の計算アシスタント役割定義
 */
export const AI_PROMPT_DERIVATION_ROLE = (tableName: string): string =>
