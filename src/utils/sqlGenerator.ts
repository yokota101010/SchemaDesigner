...
export const generateSQL = (tables: Table[], relationships: Relationship[]): string => {
  ...
  if (tableFks.length > 0) {
      ...
      tableFks.forEach(fk => {
          const childCols = fk.mappings.map((m: any) => m.childColName).join(', ');
          const parentCols = fk.mappings.map((m: any) => m.parentColName).join(', ');
          const constraintName = `fk_${fk.fromTable}_to_${fk.toTable}_` + fk.mappings.map((m: any) => m.childColName).join('_');
