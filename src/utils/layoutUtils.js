 ...
 */
export const generateBusOrthogonalPath = (startX, startY, midX, endX, endY, isChildBelow) => {
 ...
 */
export const calculateRelationshipPath = (rel, tables, relationships, viewOffset, draggingId = null, dragPos = null) => {
 ...
 */
export const isOneToOneRelationship = (rel, tables) => {
    const fromTable = tables.find(t => t.id === rel.from);
    const toTable = tables.find(t => t.id === rel.to);
 ...
 */
export const generateLoopOrthogonalPath = (startX, startY, midX, endX, endY) => {
    const r = 8; // 角丸の半径
